...
def get_my_specifity(arrs):
    ...
    in_min=0.5
    res= np.array(list(map(lambda uniq_num: analyze_single_label(uniq_num,centers, big_mask, connected,in_min), uniqq))).astype(int)
...
def get_my_sensitivity(arrs):
    ...
    res=res.astype(int)
    return res #np.mean(res.flatten())
...
def choose_file_and_get_arr(folder_path,file_name):
    files_in_folder= os.listdir(folder_path)
    chosen_file= list(filter(lambda n: file_name in n , files_in_folder))[0]
    pathh=f"{folder_path}/{chosen_file}"
    # print(f"sssssssss pathh {pathh}")
    image = sitk.ReadImage(pathh)
    return sitk.GetArrayFromImage(image)


def get_anatomy_metr_from_files(folder_path):
        predicted_segmentation_onehot= np.stack([
                choose_file_and_get_arr(folder_path,"inferred_pz")
                ,choose_file_and_get_arr(folder_path,"inferred_tz")
                ,choose_file_and_get_arr(folder_path,"inferred_sv")
                ,choose_file_and_get_arr(folder_path,"inferred_sum")
                ])

        target= np.stack([
                choose_file_and_get_arr(folder_path,"target_pz")
                ,choose_file_and_get_arr(folder_path,"target_tz")
                ,choose_file_and_get_arr(folder_path,"target_sv")
                ,choose_file_and_get_arr(folder_path,"target_sum")
                ])
        data= np.stack([
                choose_file_and_get_arr(folder_path,"t2w")

                ])
        return dict(evaluate_single_anatomy_case(["0",predicted_segmentation_onehot,target,data]," "," ",False))

        # arrs=list(map(lambda bi: (predicted_segmentation_onehot[bi,:,:,:],target[bi,:,:,:]) ,range(shapp[0])))

        # anatomy_arrs=prep_arr_list_anatomy(predicted_segmentation_onehot,target,data,shapp[0],batch_idd)
        # res=list(map(partial(evaluate_single_anatomy_case,tempdir=tempdir,to_save_files=to_save_files,for_explore=for_explore),anatomy_arrs))    
def seva_anatomy_res():
    anatomy_cases_path= "/workspaces/konwersjaJsonData/data/for_explore_anatomy"
    anatomy_cases= os.listdir(anatomy_cases_path)
    anatomy_cases= list(map(lambda n : f"{anatomy_cases_path}/{n}",anatomy_cases))
    results = list(map(get_anatomy_metr_from_files,anatomy_cases ))
    df= pd.DataFrame(results)
    df.to_csv("/workspaces/konwersjaJsonData/present_results/anatomyRes.csv")



def get_lesion_metr_from_files(folder_path):
        curr_in= choose_file_and_get_arr(folder_path,"curr_in")
        curr_twos= choose_file_and_get_arr(folder_path,"curr_twos")
        inferred= choose_file_and_get_arr(folder_path,"inferred")
        curr_bigger_mask= choose_file_and_get_arr(folder_path,"curr_bigger_mask")

        adc= choose_file_and_get_arr(folder_path,"adc")
        hbv= choose_file_and_get_arr(folder_path,"hbv")
        t2w= choose_file_and_get_arr(folder_path,"t2w")

        arrs= curr_in,curr_twos,inferred,curr_bigger_mask,t2w
...
def seva_lesions_res():
    ...
    results=[]
    with mp.Pool(processes = mp.cpu_count()) as pool:
        results = pool.map(get_lesion_metr_from_files,lesion_cases )
    sensitivity= list(map(lambda el: np.mean(np.array([el[0]])).flatten() ,results))
    specificity= list(map(lambda el: np.mean(np.array([el[1]])).flatten() ,results))
    ss=list(filter(lambda el:el>-1 ,np.array(sensitivity).flatten()))
    print(ss)
    sensitivity= np.concatenate(sensitivity)
    specificity= np.concatenate(specificity)
    print(f"sssssssss sensitivity {np.mean(ss)} {np.std(ss)} specificity {np.mean(specificity)} {np.std(specificity)}")
    df= pd.DataFrame()
    df["res"]=sensitivity
    df.to_csv("/workspaces/konwersjaJsonData/present_results/lesionRes_sensitivity.csv")
    df= pd.DataFrame()
    df["res"]=specificity
    df.to_csv("/workspaces/konwersjaJsonData/present_results/lesionRes_specificity.csv")

# aa=get_lesion_metr_from_files("/workspaces/konwersjaJsonData/data/for_explore_lesion_new/0.5_504")
# print(aa)

seva_lesions_res()

# aa=get_anatomy_metr_from_files("/workspaces/konwersjaJsonData/data/for_explore_anatomy/0.923586032652725_270500")
# print(aa)
#