...
def modify_bool_arrs(current_row,boolArrs ,indicies_to_mod ,labelsOfIntrest_inner,indicies_around):
    prev=0
    currLen=len(indicies_to_mod)
    indicies_to_mod_in= indicies_to_mod
    while( (len(indicies_to_mod)>0) and (currLen-prev >0) ):#condition needed to make recursion possible
        augmented_indicies = list(map(lambda cart_index: find_new_label(cart_index,boolArrs,labelsOfIntrest_inner,indicies_around),indicies_to_mod_in))
        indiciesToMod= list( filter( lambda tupl: tupl[3]>=0  ,augmented_indicies))

        indicies_to_mod_in= list( filter( lambda tupl: tupl[3]<0  ,augmented_indicies))
        ...
        prev= currLen
        currLen=len(indicies_to_mod_in)   

        #indicies_to_mod_in=indiciesToGoFuther
...
def dilatate_erode_conditionally(files_df,labelsOfIntrest,prostateLab ,annot):
    ...
    #list(map(partial(grow_labels,labelsOfIntrest=labelsOfIntrest,indicies_around=indicies_around,annot=annot,prostateLab=prostateLab), list(frame_of_intr.iterrows())))
