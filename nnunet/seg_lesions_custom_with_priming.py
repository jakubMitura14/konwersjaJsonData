...
non_mri_inputs=[new_col_name,"pz_noSeg",'tz_noSeg','priming']
...
def mod_path(pathh,lab_num):
    # res= pathh.replace('0.nii.gz',f"{lab_num}.nii.gz") 
    res= pathh.replace('0_',f"{lab_num}_") 
    print(f"iii {res}")
    return res
...
def save_outputs(label_new_path,out_pathsDict, lab_data,adc_image,hbv_image,t2w_image,pz_image,tz_image,min_z,min_y,min_x,max_z,max_x,max_y,t2w_image_orig):
    ...
    path_t2w=mod_path(out_pathsDict['t2w'],lesion_num)
    path_pz_noSeg=mod_path(out_pathsDict['pz_noSeg'],lesion_num)
    ...
    #for priming
    writer = sitk.ImageFileWriter()
    writer.SetFileName(path_priming)
    writer.Execute(label_image)


    writer = sitk.ImageFileWriter()
    writer.SetFileName(path_adc)
    writer.Execute(adc_image)

    writer = sitk.ImageFileWriter()
    writer.SetFileName(path_hbv)
    writer.Execute(hbv_image)

    writer = sitk.ImageFileWriter()
    writer.SetFileName(path_t2w)
    writer.Execute(t2w_image)

    writer = sitk.ImageFileWriter()
    writer.SetFileName(path_pz_noSeg)
    writer.Execute(pz_image)

    writer = sitk.ImageFileWriter()
    writer.SetFileName(path_tz_noSeg)
...
ids= list(filter(lambda el: el!=" ",ids))
...
# my_proj_name="seg lesions debug" tag="priming test" my_proj_desc="l4a test" nnUNetv2_train 101 3d_lowres 0 -tr Main_trainer_pl
