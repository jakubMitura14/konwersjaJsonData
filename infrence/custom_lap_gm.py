...
class LapGM:
    """
    modification of original LapGM that deals with nasty nans
    ...
    def estimate_parameters(self, image: Array[float, ('M', '...')], 
        ...
        log_image = np.nan_to_num(log_image, copy=True, nan=0.0, posinf=0.0, neginf=0.0).astype(float)
        print(f"llllllllllllllll {np.mean(log_image)}")
